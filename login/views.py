...
from helpers.navbar_helpers import memberClubList_function, thingys_available_function
from helpers.navbar_helpers import loaned_thingys_function, borrowed_thingys_function
from helpers.navbar_helpers import my_thingys_function, requests_function
...
def welcome(request):
    ...
    context["page_title"] = _("Magenta Backpack - Welcome")
    context["bookCount"]  = Thingy.objects.filter(thingy_type='BK').count()
    context["musicCount"] = Thingy.objects.filter(thingy_type='MU').count()
    context["movieCount"] = Thingy.objects.filter(thingy_type='MV').count()
    context["gameCount"]  = Thingy.objects.filter(thingy_type='GM').count()
